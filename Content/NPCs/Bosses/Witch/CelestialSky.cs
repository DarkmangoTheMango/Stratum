...
public class CelestialSkyScene : ModSceneEffect
{
...
// Don't look at this code, it's a mess and needs refactoring
public class CelestialSky : CustomSky
{
    ...
    const float StarSpeed = 3f;
    const float TwinkleStrength = 0.25f;
    const float TwinkleBase = 0.75f;
    const int OffscreenBuffer = 100;
    float intensity;
    ...
    static void DrawBackground(Action<Effect> configureShader)
    {
        Effect shader = ShaderManager.CelestialSky.Value;
        ...
        Texture2D Texture = ModContent.Request<Texture2D>(AssetUtils.AssetPath + "/Textures/Noise/TurbulentNoise").Value;
